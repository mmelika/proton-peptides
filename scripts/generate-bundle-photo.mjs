/**
 * Generate a new Retatrutide Starter Bundle vial photo matching existing product style.
 * Uses bpc-157.png as the style reference.
 *
 * Usage:
 *   GEMINI_API_KEY=... node scripts/generate-bundle-photo.mjs
 */
...
const outPath = join(DIR, 'retatrutide-starter-bundle.png')
...
const prompt =
  ...
  `"ProtonPeptides" (top, small text), "Retatrutide" (large, bold, center), "Starter Bundle" (below product name in smaller text), "10MG" (below that), ` +
...
console.log('Generating Retatrutide Starter Bundle photo via Gemini...')
...
console.log(`✓ saved retatrutide-starter-bundle.png`)
